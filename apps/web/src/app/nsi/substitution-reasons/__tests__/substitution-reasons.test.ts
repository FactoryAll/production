...
describe('toggleSubstitutionReasonActive', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('toggles active flag and writes audit', async () => {
    ...
    const { toggleSubstitutionReasonActive } = await import('../actions');
    const updated = await toggleSubstitutionReasonActive('sr-1');
